...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd,const_cast<WCHAR*>(L"../Project1/color.vs"), const_cast<WCHAR*>(L"../Project1/color.ps"));
	if (!result)
	{
		return false;
	}
...
void ColorShaderClass::SetShaderMode(ShaderMode mode)
{
	shaderMode = mode;
}
	...
	psFilename)
{
	...
	//===== Create Shader Bright====

	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "ColorPixelShaderDark", "ps_5_0",
		D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
	...
	result = device->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
		pixelShaderBuffer->GetBufferSize(), NULL, &m_pixelShader_dark);
	if (FAILED(result))
	{
		return false;
	}
...
void ColorShaderClass::ShutdownShader()
{
	...
	if (m_pixelShader_dark)
	{
		m_pixelShader_dark->Release();
		m_pixelShader_dark = 0;
	}
...
void ColorShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	case NORMAL:
		...
		break;
	case DARK:
		deviceContext->PSSetShader(m_pixelShader_dark, NULL, 0);
		break;
	}
	// Render the triangle.
