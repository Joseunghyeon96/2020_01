...
bool TextClass::SetWorldTime(int time, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, timeString, 100, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetDronSpeed(int speed, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char speedString[16];
	bool result;
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, speedString, 100, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
