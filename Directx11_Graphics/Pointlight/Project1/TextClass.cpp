	...
	screenWidth, int screenHeight, D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, const_cast<char*>("../Project1/data/fontdata.txt"), const_cast<WCHAR*>(L"../Project1/data/font.dds"));
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, const_cast<char*>("Hello"), 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, const_cast<char*>( "Goodbye"), 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
